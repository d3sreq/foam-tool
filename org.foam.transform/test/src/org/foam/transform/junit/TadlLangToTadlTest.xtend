...
class TadlLangToTadlTest {
	...
	@Test def void parseOpenClose() {
		...
		val varDefOpen = propFac.createVariableDefinition => [
			name = "open"
		]
		val varDefClose = propFac.createVariableDefinition => [
			name = "close"
		]
		
		val Template template = tadllang2tadl.parse(openClose)
