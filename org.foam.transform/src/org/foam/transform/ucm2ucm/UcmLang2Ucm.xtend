...
class UcmLang2Ucm {
	...
	def private parseIsPrimary(LinkedList<String> lines) {
		...
		val pattern = Pattern.compile(PRIMARY + "\\s+(\\w+)")
	...
	def private Scenario parseScenario(LinkedList<String> lines, String prefix) {
		...
		while (!lines.empty) {
			...
			if (labelTextAnnotations.label != expectedLabel) {
				val message = "Bad step label - expected " + expectedLabel + " at line \"" + labelTextAnnotations.annotatedText.text + "\""
	...
	def private AnnotatedText separateTextAndAnnotations(String textWithAnnotations) {
		...
		// get annotations
		val List<Annotation> annotations = #[]
		if (hasAnnotations) {
			do {
				val content = matcher.group(1)
				val parts = content.split(":")
				annotations += annotationFactory.createUnknownAnnotation => [
