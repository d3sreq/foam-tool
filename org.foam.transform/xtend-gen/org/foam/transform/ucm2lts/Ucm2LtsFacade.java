...
public class Ucm2LtsFacade {
  ...
  @Deprecated
  private static void reduction(final Automaton automaton) {
    final HashMap<Pair<Step,StateType>,State> mapping = Ucm2LtsFacade.createStepStateType2StateMap(automaton);
    final HasTemporalAnnotation hasTemporal = new HasTemporalAnnotation();
    EList<Transition> _transitions = automaton.getTransitions();
    final ExistsTransitionWithMarkEndingInThisState existsTransitionWithMarkEndingInThisState = new ExistsTransitionWithMarkEndingInThisState(_transitions);
    final Predicate<State> stepStateToRemovePredicate = Ucm2LtsFacade.createStepStateToRemovePredicate(automaton, mapping);
    Predicate<State> _not = Predicates.<State>not(hasTemporal);
    Predicate<State> _not_1 = Predicates.<State>not(existsTransitionWithMarkEndingInThisState);
    final Predicate<State> reductionPredicate = Predicates.<State>and(_not, _not_1, stepStateToRemovePredicate);
    final LtsReduction ltsReduction = new LtsReduction(reductionPredicate);
    MoveAnnotationsWithTypeListener _moveAnnotationsWithTypeListener = new MoveAnnotationsWithTypeListener(Guard.class);
    ltsReduction.addStateReducedEventListener(_moveAnnotationsWithTypeListener);
    MoveAnnotationsWithTypeListener _moveAnnotationsWithTypeListener_1 = new MoveAnnotationsWithTypeListener(Action.class);
    ltsReduction.addStateReducedEventListener(_moveAnnotationsWithTypeListener_1);
