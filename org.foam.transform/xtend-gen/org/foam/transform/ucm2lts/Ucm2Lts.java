        ...
        for (final Step step : _allSteps) {
          {
            ...
            StateType[] _values = StateType.values();
            for (final StateType stateType : _values) {
              {
        ...
        for (final Include include : _filter) {
          {
            ...
            String _plus = ("incl_" + _id);
            String _plus_1 = (_plus + "_from_");
            String _id_1 = useCase.getId();
            String _plus_2 = (_plus_1 + _id_1);
            String _plus_3 = (_plus_2 + "_step_");
            String _label = step.getLabel();
            String _plus_4 = (_plus_3 + _label);
            varDef.setName(_plus_4);
            ...
            final ArrayList<State> returnStates = new ArrayList<State>();
            {
              ...
              if (_not) {
                ...
                _builder.append("The included use-case ");
                UseCase _inlinedUseCase_2 = include.getInlinedUseCase();
                String _id_2 = _inlinedUseCase_2.getId();
                _builder.append(_id_2, "");
                _builder.append(" contains \"goto\" annotation, therefore we won\'t add a \"return\" transition to ");
                String _id_3 = useCase.getId();
                _builder.append(_id_3, "");
                this.debug(Ucm2Lts.logger, _builder.toString());
              }
            }
            UseCase _inlinedUseCase_2 = include.getInlinedUseCase();
            Iterable<Step> _allSteps_1 = UcmUtils.allSteps(_inlinedUseCase_2);
            final Function1<Step,Boolean> _function_3 = new Function1<Step,Boolean>() {
            ...
            Iterable<Step> _filter_1 = IterableExtensions.<Step>filter(_allSteps_1, _function_3);
            final Procedure1<Step> _function_4 = new Procedure1<Step>() {
              public void apply(final Step it) {
                ...
                _builder.append(_id, "");
                Ucm2Lts.this.debug(Ucm2Lts.logger, _builder.toString());
                Pair<Step,StateType> _mappedTo = Pair.<Step, StateType>of(it, StateType.OUT);
                State _get = stepToStateMap.get(_mappedTo);
                returnStates.add(_get);
              }
            };
            IterableExtensions.<Step>forEach(_filter_1, _function_4);
            for (final State returnState : returnStates) {
              {
                StringConcatenation _builder = new StringConcatenation();
                _builder.append("Adding \"return\" transition: ");
                String _id_2 = returnState.getId();
                _builder.append(_id_2, "");
                _builder.append("->");
                String _id_3 = stepExtState.getId();
                _builder.append(_id_3, "");
                this.debug(Ucm2Lts.logger, _builder.toString());
                final Transition returnTransition = this.addTransition(returnState, stepExtState, resultAutomaton, transitionMap);
                EList<Annotation> _annotations_4 = returnTransition.getAnnotations();
                Action _createAction_1 = this.verificationFactory.createAction();
                final Procedure1<Action> _function_5 = new Procedure1<Action>() {
                  public void apply(final Action it) {
                    it.setVariableDefinition(varDef);
                    it.setValue(false);
                  }
                };
                Action _doubleArrow_3 = ObjectExtensions.<Action>operator_doubleArrow(_createAction_1, _function_5);
                _annotations_4.add(_doubleArrow_3);
                ...
                final Procedure1<Guard> _function_6 = new Procedure1<Guard>() {
                  public void apply(final Guard it) {
                ...
                Guard _doubleArrow_4 = ObjectExtensions.<Guard>operator_doubleArrow(_createGuard_1, _function_6);
                _annotations_5.add(_doubleArrow_4);
              }
            }
          }
        }
