...
import utils.naming.NameService
...
class Ucm2Lts {
		...
		for (useCase : useCases) {
			for (entry : useCase.branches) {
				...
				for (branch : branches) {
					...
					val guards = EcoreUtil::copyAll(firstStep.annotations.filter(typeof(Guard)).toList)
	...
	def void connectContinuationFromScenarios(	Automaton							resultAutomaton,
		...
		val isGoto = Predicates::instanceOf(typeof(Goto))
	...
	def includeResolution(	Automaton							resultAutomaton,
		...
		for (useCase : useCases) {
			for (step : useCase.allSteps) {
				...
				// throw exception instead ?
				for (include : step.annotations.filter(typeof(Include))) {
